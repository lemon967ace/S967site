...
export function createTemplateBuildingController({ engine, history, onChange = () => {}, onDirty = () => {}, onEnterMode = () => {} }) {
  ...
  const api = {
    selectPalette(typeId) { onEnterMode("building"); selectedTypeId = typeId; selectedBuildingId = null; mode = "place"; preview = null; changed(); },
    ...
    startMove() { if (!api.getSelectedBuilding()) return false; onEnterMode("building"); mode = "move"; preview = null; changed(); return true; },
...
export function createTemplateRangeController({ engine, history, onChange = () => {}, onDirty = () => {}, onEnterMode = () => {} }) {
  let mode = "select", settings = null, startCell = null, previewCells = [], selectedRangeId = null;
  const changed = () => { onDirty(!history.isAtSavedState()); onChange(api.getState()); };
  const mutate = (fn, description) => command(engine, history, fn, description, changed);
  const api = {
    startCreate(value) { onEnterMode("range"); mode = "rangeCreate"; settings = { kind: value.kind, color: value.color }; startCell = null; previewCells = []; selectedRangeId = null; changed(); },
    createRectangle(value, start, end) { onEnterMode("range"); const cells = rectangleCells(start, end); const result = mutate(() => engine.addRange({ kind: value.kind, color: value.color, cells }), "fixedRangeCreate"); selectedRangeId = result.id; mode = "select"; settings = null; startCell = null; previewCells = []; return result; },
