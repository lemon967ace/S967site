...
test("all four editor entry paths call the engine", async () => {
  ...
  assert.match(html, /PNSMapEngine\.loadDocument\s*\(\s*documentData\s*\)/);
  assert.ok((html.match(/mountMapRenderer\(\);/g) ?? []).length >= 5);
