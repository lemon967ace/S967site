...
export function serializeDocument(document) {
  ...
  return structuredCloneSafe({ format: DOCUMENT_FORMAT, version: DOCUMENT_VERSION, title: document.title, map: MAP_INFO,
    ...
    fixed_buildings: document.fixedBuildings.map(item => ({ id: item.id, name: item.name, type_id: item.typeId, x: item.x, y: item.y })),
    fixed_ranges: document.fixedRanges.map(item => ({ id: item.id, kind: item.kind, color: item.color, cells: item.cells.map(cell => [...cell]) })),
...
export function parseDocument(input) {
  ...
  try {
    const document = new MapDocument({ title: requireString(data.title, "title"),
      ...
      fixedBuildings: (data.fixed_buildings ?? []).map(item => new FixedBuilding({ id: requireString(item?.id, "fixed building id"), name: requireString(item?.name, "fixed building name"), type_id: requireString(item?.type_id, "fixed building type_id"), x: requireInteger(item?.x, "fixed building x"), y: requireInteger(item?.y, "fixed building y"), width: (data.fixed_building_types ?? []).find(type => type?.id === item?.type_id)?.width, height: (data.fixed_building_types ?? []).find(type => type?.id === item?.type_id)?.height })),
      fixedRanges: (data.fixed_ranges ?? []).map(item => new FixedRange({ id: requireString(item?.id, "fixed range id"), kind: requireString(item?.kind, "fixed range kind"), color: requireString(item?.color, "fixed range color"), cells: item?.cells })),
