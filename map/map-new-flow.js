(function (root, factory) {
  ...
  function templateName(template, language) {
    ...
    return typeof template?.[field] === "string"
      ? template[field]
      : "";
  }
  ...
  function editorUrl({ title, selection }) {
    ...
    if (!trimmed) {
      throw new TypeError("MAP_NAME_REQUIRED");
    }

    if ([...trimmed].length > 100) {
      throw new RangeError("MAP_NAME_TOO_LONG");
    }

    /*
      New maps must always be created from the administrator-designated
      default fixed-map template. "empty" is deliberately rejected.
    */
    if (
